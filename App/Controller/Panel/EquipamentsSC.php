...
class EquipamentsSC extends Page
{
    private static function getEquipamentsItems($request, &$obPagination)
    {
        $itens = '';

        $quantidadeTotal = EntityEquipaments::getEquipaments("uf = 'SC' AND NOT disp_atual = 'Disponível'", null, null, "COUNT(*) as qtd")->fetchObject()->qtd;
        ...
        //CONDIÇÕES SQL
        $condicoes = [
            strlen($search) ? "uf = 'SC' AND (n_terminal LIKE '%$search%' OR req LIKE '%$search%' OR wo LIKE '%$search%') AND NOT disp_atual = 'Disponível'" : "uf = 'SC' AND NOT disp_atual = 'Disponível'"
        ];
        ...
        while($obEquipaments = $results->fetchObject(EntityEquipaments::class)) {
            ...
            $itens .= View::render('painel/modules/equipamentsSC/item',[
    ...
    {
        $content = View::render('painel/modules/equipamentsSC/index',[
            'itens' => self::getEquipamentsItems($request, $obPagination),
            'pagination' => parent::getPagination($request, $obPagination)
        ]);

        return parent::getPanel('Terminais', $content, 'terminaisSC');
    }

    public static function getImportChamados($request)
    {
        return View::render('painel/modules/equipamentsRS/importChamados',[
    ...
    public static function setImportChamados($request)
    {
        if(isset($_POST["ImportChamados"])){
            ...
            if($_FILES['importFileChamados']['size'] > 0) {
                ...
                $request->getRouter()->redirect('/equipamentsSC?status=imported');
            }
        }
    }
    ...
    {
        return View::render('painel/modules/equipamentsRS/importSimga',[
            'title' => 'Importar CSV'
        ]);
    }
